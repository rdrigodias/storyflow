...
interface ApiKeyModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (apiKey: string) => Promise<void>; // o salvamento/validação acontece fora (backend)
  canClose: boolean;
}
...
export function ApiKeyModal({ isOpen, onClose, onSave, canClose }: ApiKeyModalProps) {
  ...
  // Ao abrir: carrega do localStorage para exibir como ******** (password) e permitir copiar/editar.
  useEffect(() => {
    if (isOpen) {
      const stored =
    ...
    // Ao fechar: mantém o comportamento atual de limpar estado (não altera backend)
    setKeyInput("");
    setShowKey(false);
    setLoading(false);
    setStatus("idle");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // delega para quem chamou salvar/validar no backend
      await onSave(keyInput.trim());

      // garante persistência local (UX)
      localStorage.setItem("googleApiKey", keyInput.trim());
  ...
  const handleCopy = async () => {
    try {
      ...
      setStatus("success");
      setMessage("API Key copiada.");
    } catch {
      setStatus("error");
      setMessage("Não foi possível copiar (permissão do navegador).");
    }
  };
  ...
  const handleRemoveLocal = () => {
    ...
    setShowKey(false);
    setStatus("success");
    setMessage("API Key removida deste navegador.");
  };
        ...
        <div className="mt-4">
          ...
          <div className="mt-2 relative">
            <input
              ...
              className="w-full rounded-lg bg-gray-950 border border-gray-800 px-3 py-2 pr-28 text-white outline-none focus:border-indigo-600"
            />
