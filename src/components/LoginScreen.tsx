...
export function LoginScreen() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = data?.error || data?.message || 'Erro';
        throw new Error(msg);
      }
    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
      <div className="bg-gray-900 p-8 rounded-2xl shadow-2xl border border-gray-800 w-full max-w-md relative overflow-hidden">
        ...
        <h1 className="text-3xl font-black text-white mb-2 text-center tracking-tight">
          StoryFlow <span className="text-indigo-500">AI</span>
        </h1>
        ...
        {error && (
          <div className="text-red-400 text-xs font-bold text-center mb-4">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 p-3 bg-gray-800 border border-gray-700 rounded-lg text-white outline-none focus:border-indigo-500"
            />
          </div>
            ...
            <>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 p-3 bg-gray-800 border border-gray-700 rounded-lg text-white outline-none focus:border-emerald-500"
                />
              </div>
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="w-full pl-10 p-3 bg-gray-800 border border-gray-700 rounded-lg text-white outline-none focus:border-purple-500"
                >
                  <option value="PLAN_30_DAYS">🟢 Mensal</option>
                  <option value="PLAN_3_MONTHS">🔵 Trimestral</option>
                  <option value="PLAN_6_MONTHS">🟣 Semestral</option>
                  <option value="PLAN_1_YEAR">🟡 Anual</option>
                </select>
              </div>
            </>
          )}

          <button className="w-full bg-indigo-600 hover:bg-indigo-500 text-white p-3 rounded-lg font-bold shadow-lg transition-all mt-2">
            {isRegister ? 'SOLICITAR ACESSO' : 'ENTRAR'}
          </button>
        </form>
        ...
        <button
          ...
          className="w-full mt-4 text-gray-500 text-xs font-bold hover:text-white uppercase"
        >
