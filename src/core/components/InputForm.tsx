...
export function InputForm({ onSubmit, isLoading }: InputFormProps) {
  ...
  // Novo estado para Velocidade de Geração
  type SpeedOption = 'slow' | 'medium' | 'fast';
  const [generationSpeed, setGenerationSpeed] = useState<SpeedOption>('medium');
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Mapeamento de velocidade para delay em ms
    const delayMap: Record<SpeedOption, number> = {
        fast: 2000,
        medium: 5000,
        slow: 9000
    };
    
    const delay = delayMap[generationSpeed];
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-800/50 p-6 rounded-lg shadow-md border border-gray-700 space-y-8">
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1">
            <label className="block text-lg font-bold text-gray-300 mb-3 uppercase tracking-wider">
            ⚙️ Velocidade na Geração
            </label>
            <div className="grid grid-cols-3 gap-2">
                <button
                    type="button"
                    onClick={() => setGenerationSpeed('slow')}
                    className={`p-3 rounded-md border transition-all flex flex-col items-center justify-center gap-1 ${generationSpeed === 'slow' ? 'bg-indigo-900/50 border-indigo-400 text-white shadow-lg' : 'bg-gray-900 border-gray-700 text-gray-500 hover:border-gray-600'}`}
                >
                    <span className="text-xs font-black uppercase">Lento</span>
                    <span className="text-[10px] opacity-70">9s/img</span>
                </button>
                <button
                    type="button"
                    onClick={() => setGenerationSpeed('medium')}
                    className={`p-3 rounded-md border transition-all flex flex-col items-center justify-center gap-1 ${generationSpeed === 'medium' ? 'bg-indigo-900/50 border-indigo-400 text-white shadow-lg' : 'bg-gray-900 border-gray-700 text-gray-500 hover:border-gray-600'}`}
                >
                    <span className="text-xs font-black uppercase">Médio</span>
                    <span className="text-[10px] opacity-70">5s/img</span>
                </button>
                <button
                    type="button"
                    onClick={() => setGenerationSpeed('fast')}
                    className={`p-3 rounded-md border transition-all flex flex-col items-center justify-center gap-1 ${generationSpeed === 'fast' ? 'bg-indigo-900/50 border-indigo-400 text-white shadow-lg' : 'bg-gray-900 border-gray-700 text-gray-500 hover:border-gray-600'}`}
                >
                    <span className="text-xs font-black uppercase">Rápido</span>
                    <span className="text-[10px] opacity-70">2s/img</span>
                </button>
            </div>
        </div>
      </div>
